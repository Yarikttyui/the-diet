...
## Используемые основные технологии и библиотеки
...
* **Jetpack Compose**
````
Современный способ создания пользовательских интерфейсов на Android.
Вместо XML всё пишется на Kotlin.
Упрощает разработку, позволяет создавать красивые и анимированные интерфейсы быстро.
````
---

* **Room (AndroidX)**
````
Библиотека для работы с локальной базой данных SQLite.
Позволяет удобно хранить и извлекать данные.
Работает через аннотации и предоставляет слой абстракции над SQL.
````
---

* **Coroutines**
````
Функциональность Kotlin для работы с асинхронными задачами.
Позволяет выполнять запросы в фоне (например, к базе данных), не блокируя интерфейс.
Лёгкие, удобные, лучше чем AsyncTask или RxJava.
````
---

* **Material3 (Compose Material Design)**
````
Набор визуальных компонентов от Google, соответствующий последней версии дизайна Material You.
Выглядит современно, поддерживает тёмную тему, адаптацию под пользователя и анимации.
Используется в Jetpack Compose как стандарт стиля.
````
---
